// bin/routes_embrodry/expenses_embrodry_api.dart
import 'dart:convert';
import 'package:shelf/shelf.dart';
import 'package:shelf_router/shelf_router.dart';
import 'package:postgres/postgres.dart';

Router getEmbrodryExpensesRoutes(PostgreSQLConnection db) {
  final router = Router();
  ...
  router.get('/', (Request request) async {
    ...
    final cond = <String>[], subs = <String, dynamic>{};
    ...
    final rows = await db.query('''
      ...
      FROM embroidery.expenses e
      LEFT JOIN embroidery.material_types mt ON mt.id = e.material_type_id
      LEFT JOIN embroidery.materials      m  ON m.id  = e.material_id
      $where
  ...
  router.post('/', (Request request) async {
    try {
      ...
      if (type == null) return Response(400, body: jsonEncode({'error': 'expense_type required'}));
      if (type == 'transport') return Response(400, body: jsonEncode({'error': 'transport is hidden'}));

      final desc = body['description'] as String? ?? '';
      final date = body['expense_date'] != null
      ...
      return Response(201, body: jsonEncode({'id': res.first[0]}), headers: {'Content-Type': 'application/json'});
    } catch (e) {
  ...
  router.put('/<id>', (Request request, String id) async {
    try {
      ...
      if (count == 0) {
        return Response.notFound(jsonEncode({'error': 'Expense not found'}), headers: {'Content-Type': 'application/json'});
      }
  ...
  router.delete('/<id>', (Request request, String id) async {
    final cnt = await db.execute(
      'DELETE FROM embroidery.expenses WHERE id=@id',
      substitutionValues: {'id': int.parse(id)},
    );
    if (cnt == 0) {
      return Response.notFound(jsonEncode({'error': 'Expense not found'}), headers: {'Content-Type': 'application/json'});
