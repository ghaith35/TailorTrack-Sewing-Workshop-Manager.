// lib/design/design_expenses_section.dart
...
class DesignExpensesSection extends StatefulWidget {
  ...
  @override
  State<DesignExpensesSection> createState() => _DesignExpensesSectionState();
}
...
class _DesignExpensesSectionState extends State<DesignExpensesSection> {
  ...
  Future<void> _addOrEdit({Map? existing}) async {
    ...
    int? selTypeId = existing?['material_type_id'];
    int? selMatId  = existing?['material_id'];
    final qtyCtl   = TextEditingController(text: existing?['quantity']?.toString() ?? '');
    final priceCtl = TextEditingController(
      text: (existing?['unit_price'] as num?)?.toStringAsFixed(2) ?? '0.00',
    );
                  ...
                  try {
                    ...
                    if (existing == null) {
                      ...
                      if (resp.statusCode == 200) {
                        ...
                        _snack('فشل التحديث: ${resp.body}', err: true);
                      }
                    }
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                if (t == 'custom')
                  _row('الوصف:', (e['description'] ?? '-').toString()),
                if (t == 'raw_materials') ...[
                  const Divider(),
