...
class _SewingSalesSectionState extends State<SewingSalesSection> {
  ...
  Future<void> _showModelImageDialog(String? imageUrl) async {
    if (imageUrl == null || imageUrl.isEmpty) {
      _showSnackBar('لا توجد صورة متاحة لهذا المنتج', color: Colors.orange);
      return;
    }
                          ...
                          if (modelId != null)
                            IconButton(
                              ...
                              onPressed: () {
                                ...
                                _showModelImageDialog(selectedModel['image_url']);
                              },
                            ),
                        ],
                      ),
                              ...
                              ...items.asMap().entries.map((e) {
                                ...
                                return ListTile(
                                  ...
                                  subtitle: Column(
                                    ...
                                    children: [
                                      Text(
                                        'المتاح: ${it['available_quantity']}${role == "Admin" || role == "SuperAdmin" ? ", التكلفة: ${it['cost_price']}" : ""}',
                                      ),
                                      Text(
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  dialogSetState(() {
                    ...
                    productsError =
                        items.isEmpty ? 'يرجى إضافة منتج واحد على الأقل' : null;
  ...
  pdf.addPage(
    pw.Page(
      ...
      build: (pw.Context ctx) => pw.Column(
        ...
        children: [
          pw.Text(
            'تقرير تعاملات العميل: $clientName',
            style: pw.TextStyle(
              font: arabicFont,
              fontSize: 24,
              fontWeight: pw.FontWeight.bold,
          ...
          pw.SizedBox(height: 20),
          pw.Text(
            'رقم العميل: ${widget.clientId}',
            style: pw.TextStyle(font: arabicFont, fontSize: 16),
          ),
          if (startDate != null && endDate != null)
            pw.Text(
              'الفترة: من ${fmtDate(startDate!.toIso8601String())} إلى ${fmtDate(endDate!.toIso8601String())}',
              style: pw.TextStyle(font: arabicFont, fontSize: 16),
            ),
          pw.SizedBox(height: 20),
          pw.Text(
            ...
            children: [
              pw.TableRow(
                children: [
                  pw.Padding(
                    padding: const pw.EdgeInsets.all(8),
                    child: pw.Text('رقم الفاتورة', style: pw.TextStyle(font: arabicFont)),
                  ),
                  pw.Padding(
                    padding: const pw.EdgeInsets.all(8),
                    child: pw.Text('تاريخ التعامل', style: pw.TextStyle(font: arabicFont)),
                  ),
                  pw.Padding(
                    padding: const pw.EdgeInsets.all(8),
                    child: pw.Text('النوع', style: pw.TextStyle(font: arabicFont)),
                  ),
                  pw.Padding(
                    padding: const pw.EdgeInsets.all(8),
                    child: pw.Text('المبلغ', style: pw.TextStyle(font: arabicFont)),
                  ),
                ],
              ),
              ...clientTransactions.map((t) => pw.TableRow(
                    children: [
                      pw.Padding(
                        padding: const pw.EdgeInsets.all(8),
                        child: pw.Text(
                      ...
                      pw.Padding(
                        padding: const pw.EdgeInsets.all(8),
                        child: pw.Text(
                          '${(t['amount'] as num?)?.toStringAsFixed(2) ?? '0'} دج',
                          style: pw.TextStyle(font: arabicFont),
                        ),
                      ),
                    ],
          ...
          pw.SizedBox(height: 20),
          pw.Text(
            'إجمالي الدفعات: ${(clientTransactions.where((t) => t['type'] == 'payment').fold<double>(0.0, (s, t) => s + (t['amount'] as num)).toStringAsFixed(2))} دج',
            style: pw.TextStyle(font: arabicFont, fontSize: 16),
          ),
          pw.Text(
            'إجمالي المرتجعات: ${(clientTransactions.where((t) => t['type'] == 'return').fold<double>(0.0, (s, t) => s + (t['amount'] as num)).toStringAsFixed(2))} دج',
            style: pw.TextStyle(font: arabicFont, fontSize: 16),
          ),
        ],
      ),
    ...
    child: Column(
      ...
      children: [
        ...
        ...clientFactures.map((f) {
          final isExpanded = expandedFactureIds.contains(f['id']);
          return Card(
            margin: const EdgeInsets.symmetric(vertical: 8),
                      ...
                      children: [
                        ...
                        SingleChildScrollView(
                          ...
                          child: DataTable(
                            ...
                            rows: (f['items'] as List<dynamic>).map<DataRow>((it) {
                              final profitPerPiece = it['profit_per_piece'] as num? ?? 0;
                              return DataRow(cells: [
                                DataCell(Text(it['model_name'] ?? '')),
                                DataCell(Text('${it['quantity'] ?? 0}')),
                                DataCell(Text(
                                ...
                                if (role == 'Admin' || role == 'SuperAdmin')
                                  DataCell(
                                      Text('${profitPerPiece.toStringAsFixed(2)} دج')),
                                DataCell(Text(
                                    '${(it['line_total'] as num?)?.toStringAsFixed(2) ?? '0'} دج')),
                              ]);
                            }).toList(),
                        ...
                        const SizedBox(height: 8),
                        if (f['amount_paid_on_creation'] != null &&
                            f['amount_paid_on_creation'] > 0)
                          ListTile(
                            leading: const Icon(Icons.payment, color: Colors.green),
                            title: Text(
                                '${f['amount_paid_on_creation']?.toStringAsFixed(2) ?? '0'} دج'),
                            subtitle: Text(
                                'دفعة عند الإنشاء - تاريخ: ${fmtDate(f['facture_date'])}'),
                          ),
                        if (f['payments'] != null && (f['payments'] as List).isNotEmpty)
                          ...(f['payments'] as List).map<Widget>((p) => ListTile(
                                leading: const Icon(Icons.payment),
                                title: Text(
                                    '${p['amount_paid']?.toStringAsFixed(2) ?? '0'} دج'),
                                subtitle:
                        ...
                        if (f['returns'] != null && (f['returns'] as List).isNotEmpty) ...[
                          ...
                          SingleChildScrollView(
                            ...
                            child: DataTable(
                              ...
                              rows: (f['returns'] as List<dynamic>)
                                  .map<DataRow>((r) => DataRow(cells: [
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              ElevatedButton.icon(
                icon: const Icon(Icons.print),
                label: const Text('طباعة التعاملات'),
                onPressed: clientTransactions.isNotEmpty ? _printTransactions : null,
              ),
            ],
          ),
        ),
                        ...
                        children: [
                          ...
                          SingleChildScrollView(
                            ...
                            child: DataTable(
                              ...
                              columns: const [
                                DataColumn(label: Text('رقم الفاتورة')),
                                DataColumn(label: Text('تاريخ التعامل')),
                                DataColumn(label: Text('النوع')),
                                DataColumn(label: Text('المبلغ')),
                              ],
                              rows: clientTransactions.map<DataRow>((t) {
                                return DataRow(cells: [
                                  DataCell(
                                      Text(t['facture_id']?.toString() ?? '-')),
                                  DataCell(
                                      Text(fmtDate(t['transaction_date']))),
                                  DataCell(Text(
                                      t['type'] == 'payment' ? 'دفعة' : 'إرجاع')),
                                  DataCell(Text(
                                      '${(t['amount'] as num?)?.toStringAsFixed(2) ?? '0'} دج')),
